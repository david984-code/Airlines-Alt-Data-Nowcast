"""TSA daily checkpoint passenger volumes -- the ground-truth air-travel signal.
...
_HEADERS = {"User-Agent": "Mozilla/5.0 (research; airlines nowcast)"}
...
def fetch(force: bool = False) -> pd.Series:
    """Daily Series of TSA passengers indexed by date, back to START_DATE."""
    ...
    df = pd.concat(frames).drop_duplicates("date").sort_values("date").set_index("date")
